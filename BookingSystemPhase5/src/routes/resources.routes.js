...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    // Add log event — descriptive message for success
    const resourceId = rows[0].id;
    const created = rows[0];
    const successMessage = `✅RESOURCE_CREATED: name="${created.name}", price=${created.price}, unit="${created.price_unit}"`;
    await logEvent({
      actorUserId,
    ...
    if (err && err.code === "23505") {
      // Add log event — descriptive message for duplicate
      console.error(err);
      const duplicateMessage = `❌RESOURCE_CREATE_BLOCKED_DUPLICATE: name="${resourceName}" (409)`;
      await logEvent({
        actorUserId,
