...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "Some of your input was invalid. Please fix the fields below and try again.";
  }
  ...
  return `Your input was invalid. Please fix the following fields and try again:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg =
          "A resource with this name already exists. Please choose a different name or check the resource list.";
    ...
    const resourceName = body?.data?.name ?? "Resource";
    const msg = `Resource added successfully. "${resourceName}" is now available.`;
